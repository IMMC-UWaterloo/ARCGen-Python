...
def arcgen(
    ...
    """
    ARCGen: Calculation of a characteristic average and statistical response corridors
    using arc-length re-parameterization and signal registration. ARCGen can accomandate
    a very wide range of input signals, including non- monotonic nad hystertic signals.

    If you intend to publish work using ARCGen, please use the following citation:
        Hartlen, D.C. & Cronin, D.S. (2022) "Arc-Length Re-Parametrization and
            Signal Registration to Determine a Characteristic Average and Statistical
            Response Corridors of Biomechanical Data." Frontiers in
             Bioengineering and Biotechnology 10:843148. doi: 10.3389/fbioe.2022.843148

    ARCGen is released under a GNU GPL v3 license. No warranty or support is provided.
    The authors hold no responsibility for the validity, accuracy, or applicability of
    any results obtained from this code.

    Parameters
    ----------
    inputData:
        One of three input formats: str: a path to a csv file containing all two-column
        signals list of np.ndarrays: useful when signals are processed prior to ARCGen
    ...
    CorridorRes: int
        Size of grid (along one size) used for extracting corridors via a
            marching squares algorithm. Grid extends 120% beyond the extreme corridor
            extents. Number of points in the grid is CorridorRes^2. Default: 250
    nWarpCtrlPts: int
    ...
    NormalizeSignals: str
        Cotnrols if signal scaling is performed to prevent differences in
            extent between ordinate and abscissca from monopolizing arc-length. Options:
            'on' (Default and highly recommended), 'off'
    EllipseKFact: float
        Scales the size of elliptical confidence regions produced at each point
            of the characteristic average, hence scaling corridor extent. Default: 1.0
    MinCorridorWidth: float
        Enforces minimum corridor width as a factor of maximum standard
            deviation. Any st. dev. less than MinCorridorWidth*max(st. dev.) is replaced
            with MinCorridorWidth*max(st. dev.). Ordinate and abscissca are handled
            independently. Default: 1.0.
    resultsToFile: bool
        If True, arcgen() will write characteristic average and corridors to a
            ...
            corridors. These are output to the folder 'outputs' in the current working
    ...
    processedSignals: list of dictionaries
        List with as many entries each containing a dictionary with data and
            information used when processing input signals. Dictionary format is
            {'data': np.ndarray, 'specId': str, 'xMax': float, 'xMin': float, 'yMax':
            float, 'yMin': float, 'maxAlen': float, 'resampled': np.ndarray,
            'warpControlPoints': np.ndarray}
    debug: dictionary
        Contains information useful for debugging purposes. Dictionary format:
            {'charAvg': np.ndarray, 'stDev': np.ndarray, 'preWarpCorrArray': np.ndarray,
            'preWarpMeanCorr': float, 'warpedCorrArray': np.ndarray,
            'warpedMeanCorrScore': float}

    Notes
    -----
    Please refer to package readme, repository
    (https://github.com/IMMC-UWaterloo/ARCGen-Python), or provided article for a
    detailed description of how ARCGen functions

    It is common to see issues when running this function if the number of resampling
    points or corridor resolution is too sparse or signals exhibit significant
    variablity not accounted for through signal registration. This tends to manifest in
    either truncated corridors or the code termininating in an error. Often increasing
    resampling points or corridor resolution. Turning 'Diagnostics' to 'detailed' can
    help identify these issues.

    Computed corridors will often not extend all the way to the shared origin of input
    signals. This is because small low st. dev. at this shared point is too low to be
    captured during corridor extraction with the marching squares algorithm. There are
    two solutions to this problem. First, one could force a minimum corridors size using
    the 'MinCorridorWidth' option. Second, one could manually extend corridors in
    post-processing.

    The python version of ARCGen is generally updated less frequently than the original
    MATLAB version of ARCGen. Active developments are implimented in MATLAB prior to
    being ported to python. The MATLAB version of ARCGen can be found at
    https://github.com/IMMC-UWaterloo/ARCGen

    Very special thanks is due to Ahmed Ibrahim, my fellow Ph.D. candidate at UWaterloo,
    for his help in getting this port started.

    Copyright (c) 2022-2026 Devon C. Hartlen
    """
    ...
    if isinstance(inputData, str):
        ...
        for iSig in range(nSignal):
            ...
            # add to dictionary
            inputSignals.append(
                {"data": temp[indexNan, :], "specId": "Signal " + str(iSig + 1)}
            )
    ...
    # Now check for the other two input cases. Both start with a list check
        ...
        if isinstance(inputData[0], np.ndarray):
            ...
            for iSig in range(len(inputData)):
                inputSignals.append(
                    {"data": inputData[iSig], "specId": "Signal " + str(iSig + 1)}
                )
        ...
        # Case 3: list of dictionaries
            ...
            for entry in inputData:
                inputSignals.append({"data": entry["data"], "specId": entry["specId"]})
        else:
    ...
    if NormalizeSignals == "off":
        for signal in inputSignals:
            ...
            segments = np.sqrt(
                (temp[0:-1, 0] - temp[1:, 0]) ** 2 + (temp[0:-1, 1] - temp[1:, 1]) ** 2
            )
    ...
    if NormalizeSignals == "on":
        ...
        xBound = [
            sum([signal["xMin"] for signal in inputSignals]) / nSignal,
            sum([signal["xMax"] for signal in inputSignals]) / nSignal,
        ]
        yBound = [
            sum([signal["yMin"] for signal in inputSignals]) / nSignal,
    ...
    for signal in inputSignals:
        ...
        resampX = interpolate.interp1d(signal["data"][:, 3], signal["data"][:, 0])(
            normAlen
        )
        resampY = interpolate.interp1d(signal["data"][:, 3], signal["data"][:, 1])(
            normAlen
        )
    ...
    if nWarpCtrlPts > 0:
        ...
        if nWarp == 1:
            ...
            raise ValueError(
                "Specifying more than 15 interior warping points is not supported"
            )
        else:
            ...
            for i in range(nWarp):
                x0[
                    np.array([(i * nSignal) + np.arange(0, nSignal) + (i * (nSignal))])
                ] = ((i + 1) / (nWarp + 1)) * np.ones((nSignal))
                x0[
                    np.array(
                        [(i * nSignal) + np.arange(0, nSignal) + ((i + 1) * nSignal)]
                    )
                ] = ((i + 1) / (nWarp + 1)) * np.ones((nSignal))
            ...
            for iSignal in range(nSignal * 2):
                for iWarp in range(nWarp - 1):
                    A[
                        iSignal + (iWarp) * (nSignal * 2),
                        iSignal + ((iWarp) * (nSignal * 2)),
                    ] = 1
                    A[
                        iSignal + (iWarp) * (nSignal * 2),
                        iSignal + ((iWarp + 1) * (nSignal * 2)),
                    ] = -1

            linConstraints = optimize.LinearConstraint(
                A, -np.inf * np.ones(((nWarp - 1) * (nSignal * 2))), b
            )

        # Execute optimization and compute warped signals
        bounds = list(zip(lb, ub))
        optWarpArrayX = optimize.minimize(
            warpingObjective,
            x0,
            args=(nWarp, inputSignals, WarpingPenalty, nResamplePoints),
            method="SLSQP",
            bounds=bounds,
        ...
        warpedSignals, signalX, signalY = warpArcLength(
            optWarpArray, inputSignals, nResamplePoints
        )
    ...
    if Diagnostics == "on" or Diagnostics == "detailed":
        ...
        for iSig, signal in enumerate(inputSignals):
            ...
            if nWarpCtrlPts > 0:
                interpResults = interpolate.pchip(
                    signal["warpControlPoints"][:, 0],
                    signal["warpControlPoints"][:, 1],
                    axis=0,
                )(signal["data"][:, 3])

                ax[0][1].plot(
                    signal["data"][:, 3], interpResults, label=signal["specId"]
                )
                ax[0][1].plot(
                    signal["warpControlPoints"][:, 0],
                    signal["warpControlPoints"][:, 1],
    ...
    # Create grids based on upper and lower of characteristic average plus 125% of
    # maximum standard deviation
    scaleFact = 1.25 * EllipseKFact
    xx, yy = np.meshgrid(
        np.linspace(
            (np.min(charAvg[:, 0]) - scaleFact * np.max(stdevData[:, 0])),
            (np.max(charAvg[:, 0]) + scaleFact * np.max(stdevData[:, 0])),
            num=CorridorRes,
        ),
        np.linspace(
            (np.min(charAvg[:, 1]) - scaleFact * np.max(stdevData[:, 1])),
    ...
    # algorithm is to find the zz=1 isoline, as this represents the outer boundary of
    # all elllipses.
    #
    # Described in brief, this algorithm goes through each point, looking at its and its
    # neighbours values. There are only 16 configurations of these squares or cells.
    # Based on the configuration, add the appropriate line segments. This method uses
    # linear interpolation to increase accuracy. Initalize line segments for speed. This
    # line may cause issues, as it assumes maximum size. Bump up 10 if it does.
    ...
    for iPt in range(CorridorRes - 1):  # Rows (y-axis)
        for jPt in range(CorridorRes - 1):  # Columns (x-axis)
            ...
            if cellValue == 1:
                ...
                lineSegments[iSeg, :] = [
                    interpIso(
                        xx[iPt, jPt], zz[iPt, jPt], xx[iPt, jPt + 1], zz[iPt, jPt + 1]
                    ),
                    yy[iPt, jPt],
                    xx[iPt, jPt],
                    interpIso(
                        yy[iPt, jPt], zz[iPt, jPt], yy[iPt + 1, jPt], zz[iPt + 1, jPt]
                    ),
                ]
                ...
                lineSegments[iSeg, :] = [
                    interpIso(
                        xx[iPt, jPt + 1], zz[iPt, jPt + 1], xx[iPt, jPt], zz[iPt, jPt]
                    ),
                    yy[iPt, jPt + 1],
                    xx[iPt, jPt + 1],
                    interpIso(
    ...
    # Create a vertex connectivity table. The 1e-12 value is here because floats don't
    # round well and == will not work.
    ...
    for i in range(len(vertConn) - 1):
        ...
        vertConnToFind_ = np.any(
            (vertConn[j:, :].astype(int) == vertToFind.astype(int)), axis=1
        )
        ...
        # If we did not find an index, we either may have an open envelope or envelope
        # may be convex and loops back on itself.
        elif foundShiftedInd.size == 0:
            # Check to see if we can find the first vertex in envelope appearing again
            # (check for closure)
            vertConn_ = allEnvelopes[nEnvelopes, 0].astype(int)
            vertToFind = vertConn[vertConn_, 1]
            foundShiftedInd = find(
                np.any(vertConn[j:, :] == vertToFind, axis=1), "first"
            )
    ...
    envelope = vertices[
        vertConn[envInds[0].astype(int) : envInds[1].astype(int) + 1, 0], :
    ]

    # Divide the envelope into corridors.
    # To break the largest envelop into inner and outer corridors, we need to account
    # for several edge cases. First, we test to see if there are any intercepts of the
    # characteristic average and the largest envelope.
    ...
    # If we find only one intercept, we need to determine if the intercept is a the
    # start or end of the envelope. Then we need to extend the opposite side of the
    # characteristic average to intercept the envelope.
        ...
        aLenExtension = (
            np.abs(aLenInterval / (charAvg[0, :] - charAvg[1, :]))
            * 1.1
            * np.max(stdevData)
        )
        ...
        # If the single found point is inside the envelope, the found intercept is at
        # the end. Therefore extend the start
        if poly.inpolygon(envelope, charAvg[indexIntercept[0, 1], :]):
            iIntEnd = indexIntercept[-1, 0]
            linestart_0 = interpLin(
                0, charAvg[0, 0], aLenInterval, charAvg[1, 0], -aLenExtension
            )
            linestart_1 = interpLin(
                0, charAvg[0, 1], aLenInterval, charAvg[1, 1], -aLenExtension
            )
        ...
        # intercept is the start
            ...
            lineend_0 = interpLin(
                1 - aLenInterval, charAvg[-2, 0], 1, charAvg[-1, 0], (1 + aLenExtension)
            )
            lineend_1 = interpLin(
                1 - aLenInterval, charAvg[-2, 1], 1, charAvg[-1, 1], (1 + aLenExtension)
            )
    ...
    # If we find no intercepts, we need to extend both sides of characteristic average
    # to intercept the envelop.
    else:
        aLenInterval = 1 / nResamplePoints
        indexLength = round(0.2 * len(charAvg))
    ...
    # To divide inner or outer corridors, first determine if polygon is clockwise or
    # counter-clockwise. Then, based on which index is large, separate out inner and
    # outer corridor based on which intercept index is larger.
    ...
    # Resample corridors. Use nResamplePoints. Because corridors are non-monotonic,
    # arc-length method discussed above is used. Start with inner corridor. Magnitudes
    # are being normalized.
    segments = np.sqrt(
        ((innerCorr[0:-1, 0] - innerCorr[1:, 0]) / np.max(innerCorr[:, 0])) ** 2
        + ((innerCorr[0:-1, 1] - innerCorr[1:, 1]) / np.max(innerCorr[:, 1])) ** 2
    )
    ...
    # Outer Corridor
    segments = np.sqrt(
        ((outerCorr[0:-1, 0] - outerCorr[1:, 0]) / np.max(outerCorr[:, 0])) ** 2
        + ((outerCorr[0:-1, 1] - outerCorr[1:, 1]) / np.max(outerCorr[:, 1])) ** 2
    )
    alen = np.cumsum(np.concatenate([[0], segments]))
    alenResamp = np.linspace(0, np.max(alen), num=nResamplePoints)
    alenResamp = np.transpose(alenResamp)
    ...
    if Diagnostics == "detailed":
        ...
        axd[0].plot(
            closedEnvelope[:, 0], closedEnvelope[:, 1], c="darkgreen", linewidth=0.5
        )
    ...
    if resultsToFile:
        ...
        plt.plot(
            charAvg[:, 0], charAvg[:, 1], label="Average - ARCGen", c="black", lw=1.5
        )
        plt.plot(
            innerCorr[:, 0], innerCorr[:, 1], label="Inner Corridors", c="gold", lw=1.5
        )
        plt.plot(
            outerCorr[:, 0],
            outerCorr[:, 1],
            label="Outer Corridors",
            c="goldenrod",
            lw=1.5,
        )
...
def evalCorrScore(signalsX, signalsY):
    """
    Compute the correlation score of signals for use in signal registration. Correlation
    score used here is taken from the work of Nusholtz et al. (2009)
...
def warpingObjective(optimWarp, nCtrlPts, inputSignals, penaltyFactor, nResamplePoints):
    """
    Objective function used by optimization algorithm during signal registration with
    the goal of computing optimal control point locations. Does not perform warping,
    only evaluates objective for optimization

    Parameters
    ----------
    optimWarp: np.ndarray
        Array of warping control points for which the objective function is computed for
    nCtrlPts: int
        Number of interior control points
    inputSignals: list of dictionaries
        The internal list of dictionaries used to keep track of signal information
        inside arcgen()
    penaltyFactor: float
...
def warpArcLength(warpArray, inputSignals, nResamplePoints):
    ...
    inputSignals: np.ndarray
        The internal list of dictionaries used by arcgen() to keep track of signals and
        signal information
    nResamplePoints: int
    ...
    warpedSignals: list of np.ndarrays
        Resampled signals warped w.r.t arc-length using the control points specified in
        warpArray
    signalsX: np.ndarray
    ...
    for iSig, signal in enumerate(inputSignals):
        ...
        resampX = interpolate.interp1d(
            warpedNormAlen, temp[:, 0], kind="linear", fill_value="extrapolate"
        )(resamNormwarpedAlen)
        resampY = interpolate.interp1d(
            warpedNormAlen, temp[:, 1], kind="linear", fill_value="extrapolate"
        )(resamNormwarpedAlen)
...
def warpingPenalty(warpArray, penaltyFactor, nResamplePoints, nSignal):
    """
    Computes the combined warping penalty for all signals given the provided warping
    points and inputted penalty factor
    ...
    penaltyScores: list, array-like
        Individual penalty scores for each warping function inputted. Same length as
        nSignal
    ...
    for iSignal in range(nSignal):
        interpX = np.concatenate(
            [[0], warpArray[iSignal + nSignal, :], [1]], axis=None, dtype="float"
        )
        interpY = np.concatenate(
            [[0], warpArray[iSignal, :], [1]], axis=None, dtype="float"
        )
...
def interpIso(x1, y1, x2, y2):
    """
    Performs linear interpolation to find where x is equal to 1.0. Only used for
...
def find(array, string=None):
    """
    Returns indices and values of non-zero elements. Similar functionality to MATLAB
    find() function.

    Copyright (c) 2022-2026 Ahmed Ibrahim
    """
...
def evaluateGrid(xx, yy, charAvg, stdevData, EllipseKFact):
    """
    Evaluates the sampling grid of the marching squares algorithm used to extract
    corridors. At each sampling point, every ellipse in the characteristic average is
    computed, with the maximum value being saved.
    ...
    EllipseKFact: float
        Factor used to scale the size of the ellipes at each point in the characteristic
        average, hence scale the extent of the corridors
    ...
    optimization could be performed to improve runtime. Later profiling has shown that
    this is by no means the slowest part of arcgen(). That would be the minimization
    algorithm and polyxpoly()
    """
