...
def uniquetol(a: npt.ArrayLike, tol=1e-6):
    """
    Finds the unique elements of input array a within tolerance tol. A sorted array
    based on first column is returned.

    Tolerance check is performed between two indices, u & v, per: max(abs(a(u)-a(v))) <
    tol
    ...
    uniquetol(). MATLAB does not provide a particulary useful description of the
    algorithm they use, so this was developed largely from scratch.


    This is a quite crude and unoptimized approach, but it has proved itself fairly
    robust thoughout many useage case. It could probably be optimized, but it is not a
