...
inputPath = (
    "Mattucci_CervicalLigaments/Mattucci_AnteriorLongitudinalLigament_QuasiStatic.csv"
)
...
charAvg, innCorr, outCorr, processed, debug = arcgen(
    inputSignals, nResamplePoints=200, CorridorRes=100
)
...
(pInn,) = plt.plot(
    innCorr[:, 0], innCorr[:, 1], label="Inner Corridor", c="gold", lw=1.5
)
(pOut,) = plt.plot(
    outCorr[:, 0], outCorr[:, 1], label="Outer Corridor", c="goldenrod", lw=1.5
)
...
inputPath = (
    "Mattucci_CervicalLigaments/Mattucci_PosteriorLongitudinalLigament_QuasiStatic.csv"
)
rawCsv = np.genfromtxt(inputPath, delimiter=",", encoding=None)
nSignals = rawCsv.shape[1] / 2
inputSignals = []
