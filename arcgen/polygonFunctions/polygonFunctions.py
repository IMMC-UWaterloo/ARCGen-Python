"""
A series of functions pertaining to polygons to be used with ARCGen. These functions
serve as replacements for the original matlab functions
"""
...
def signedpolyarea(xy: npt.ArrayLike):
    """
    Computes the signed area of a polygon. When positive y is upward, positive area
    indicates clockwise. Only suitable for non-complex polygon (no self-intersections)
...
def polyarea(xy):
    """
    Determines the area of the inputted polygon. Polygon does not need to be closed.
    Only suitable for non-complex polygon (no self-intersections)
...
def inpolygon(poly: npt.ArrayLike, point: npt.ArrayLike):
    """
    Determines if the point is in the polygon. Uses ray-casting method. Not suitable for
    self-intersecting polygons, but handles non-convex polygons fine. Does not assume
    closed polygons.
...
def polyxpoly(poly1: npt.ArrayLike, poly2: npt.ArrayLike):
    """
    Computes the intersection of two polynomials. Does not assumed closed polygons.
    ...
    interInds: np.ndarray
        Two column array specifying the indices were the two polygons
            intersect. Column 1 are indices of the first polygon. Returns interpolated
            indices as floats. Use floor() or ceil() to get integer indices on either
    ...
    for i, ind in enumerate(indApproxInd):
        ...
        # 2023-05-01 (DCH) Bug with X due to array dim. Flatten ensures vector not array
        X = X.flatten()
...
def _segorientation(p1, p2, p3):
    ...
    orient = (p2[1] - p1[1]) * (p3[0] - p2[0]) - (p3[1] - p2[1]) * (p2[0] - p1[0])
